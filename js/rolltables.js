...
function titleToId(title) {
    return title.trim().replaceAll("-", "").trim().split(" ").join("_")
}
...
function catchHref() {
    if (window.location.hash) {
        ...
        console.log(hash);
        var table_id = hash.replace("#", "");
        document.getElementById(table_id).click()
        // for (let i = 0; i < current.items.length; i++) {
        //     var current_table = current.items[i];
...
function loadleftdisplay(curr_table) {
    ...
    for (var i = 0; i < current.items.length; i++) {
        ...
        //var patt = /\/\(\/g/gi;
        if (current.items[i].id === undefined) {
            current.items[i].id = titleToId(display_title);
        }
        ...
        $("#left-display-list").append(
            `<div id='${current.items[i].id}' href='${current.items[i].id}' class='list-item' listid='${i}' item="${current.items[i].title}">${display_title}</div>`
        );
    }
...
function perform_roll() {
    window.location.hash = $(".list-selected").attr("id");
